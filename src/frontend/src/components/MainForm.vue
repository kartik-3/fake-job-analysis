<!--
  This is the main form which the user will use to get predictions.
-->
...
<template>
  <v-container>
    <v-alert class="my-16 text-justify text-h4" color="blue" dense text elevation="2">
      <div class="mt-16 mb-10" style="">
        ...
        <v-row>
          <v-col cols="6">
            ...
            <v-textarea v-if="disableJobRequirements" v-model="jobRequirements" label="Job Requirements"
              @keyup="checkValid" required></v-textarea>
          </v-col>
          <v-col cols="6">
      ...
      <v-divider></v-divider>
      <v-btn :disabled="invalid" color="success" class="mt-6" :class="{ 'mb-16': result == '' }"
        style="width: 50%; margin-left: 25%;" @click="submit">
        Submit
      </v-btn>
